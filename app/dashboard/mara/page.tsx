// Página Mara refinada con scrollbar discreta y fija para History, fuentes limpias y sin overflow
'use client';
...
export default function MaraPage() {
  ...
  const handleNewChat = () => {
    setMessages([{ from: 'mara', text: 'Hey! **I’m Mara**, your AI strategist. Need help with conversions, growth or product ideas?' }]);
    setHistoryItems([]);
  };
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', background: '#fff', fontFamily: 'Inter, sans-serif', borderRadius: '1rem', overflow: 'hidden' }}>
      <aside style={{
        ...
        <div>
          <div style={{ width: '100%', height: '160px', backgroundColor: '#f3e8ff', borderRadius: '1rem', marginBottom: '1.5rem', display: 'flex', alignItems: 'center', justifyContent: 'center', color: primaryColor, fontWeight: 600 }}>
            [ Mara Image ]
          </div>
      ...
      <section style={{ flex: 1, display: 'flex', flexDirection: 'column', padding: '2rem', background: '#f5f3ff', overflow: 'hidden' }}>
        <div style={{ flexShrink: 0 }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.75rem', marginTop: '1rem' }}>
            {[ 'Audita mi tienda online', 'Sugiere mejoras de conversión', 'Recomiéndame un producto tendencia', 'Crea una estrategia de crecimiento' ].map((suggestion, idx) => (
              <button
                key={idx}
                onClick={() => {
                  if (inputRef.current) {
        ...
        <form onSubmit={handleSubmit} style={{ padding: '1rem 0 1.25rem', display: 'flex', gap: '1rem', alignItems: 'center', backgroundColor: '#f5f3ff', borderTop: '1px solid #e4e4e4' }}>
          <input
            ref={inputRef}
            name="msg"
