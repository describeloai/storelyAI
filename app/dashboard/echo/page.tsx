// Página Echo refinada como las anteriores, sin degradado, con scrollbar profesional y fuente moderna
'use client';
...
export default function EchoPage() {
  const [messages, setMessages] = useState([
    { from: 'echo', text: 'Hello! **I’m Echo**, your AI support assistant. I can help you analyze performance, answer questions, and support your customers.' },
  ]);
  ...
  const handleNewChat = () => {
    setMessages([{ from: 'echo', text: 'Hello! **I’m Echo**, your AI support assistant. I can help you analyze performance, answer questions, and support your customers.' }]);
    setHistoryItems([]);
  };
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', background: '#fff', fontFamily: 'Inter, sans-serif', borderRadius: '1rem', overflow: 'hidden' }}>
      <aside style={{
        ...
        <div>
          <div style={{ width: '100%', height: '160px', backgroundColor: '#dcfce7', borderRadius: '1rem', marginBottom: '1.5rem', display: 'flex', alignItems: 'center', justifyContent: 'center', color: primaryColor, fontWeight: 600 }}>
            [ Echo Image ]
          </div>
      ...
      <section style={{ flex: 1, display: 'flex', flexDirection: 'column', padding: '2rem', background: '#f0fdf4', overflow: 'hidden' }}>
        <div style={{ flexShrink: 0 }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.75rem', marginTop: '1rem' }}>
            {[ 'Responde una queja de cliente', 'Genera una secuencia de emails de postventa', 'Traduce mensaje al inglés', 'Sugiere mejoras en soporte al cliente' ].map((suggestion, idx) => (
              <button
                key={idx}
                onClick={() => {
                  if (inputRef.current) {
        ...
        <form onSubmit={handleSubmit} style={{ padding: '1rem 0 1.25rem', display: 'flex', gap: '1rem', alignItems: 'center', backgroundColor: '#f0fdf4', borderTop: '1px solid #e4e4e4' }}>
          <input
            ref={inputRef}
            name="msg"
