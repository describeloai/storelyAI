...
export default function Sidebar({ isSidebarOpen, onClose }: SidebarProps) {
  ...
  return (
    ...
    >
      <div onClick={() => handleClick('/dashboard/settings')} style={getLinkStyle(pathname === '/dashboard/settings')}>
        <Settings size={16} />
      ...
      <nav style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
        {links.map(({ href, label, icon: Icon }) => (
          <div key={href} onClick={() => handleClick(href)} style={getLinkStyle(pathname === href)}>
            <Icon size={16} />
            <span>{label}</span>
          </div>
        ))}
